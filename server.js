// Minimal mock of the Xtream Codes Player API for developing and testing IPTV
// client apps. Implements only the endpoints a typical client calls, and serves
// PUBLIC-DOMAIN / Creative-Commons content (Blender + Google sample films for
// VOD/episodes, Apple/Mux test HLS for "live"). Any username/password is accepted.
//
// Deploy anywhere that gives an HTTPS URL (Render, Railway, Fly, Cloudflare…).
// For the Cloudflare Workers version see worker.js + wrangler.toml.
//
...
const MOVIE_CATEGORIES = [
  { category_id: "1", category_name: "Open Movies", parent_id: 0 },
];
const SERIES_CATEGORIES = [
  { category_id: "10", category_name: "Sample Series", parent_id: 0 },
];
const LIVE_CATEGORIES = [
  { category_id: "20", category_name: "Test Channels", parent_id: 0 },
];
...
// ffprobe-style blocks required by MovieDetail.Info (strict decode).
const DISPOSITION = {
...
function accountInfo(username, password) {
  return {
    ...
    server_info: {
      url: "", port: "80", https_port: "443", server_protocol: "https",
    },
  };
}

app.get("/player_api.php", (req, res) => {
  const { username = "review", password = "review", action } = req.query;

  switch (action) {
    case undefined:                 return res.json(accountInfo(username, password));
    case "get_vod_categories":      return res.json(MOVIE_CATEGORIES);
    case "get_vod_streams":         return res.json(MOVIES);
    case "get_series_categories":   return res.json(SERIES_CATEGORIES);
    case "get_series":              return res.json(SERIES);
    case "get_live_categories":     return res.json(LIVE_CATEGORIES);
    case "get_live_streams":        return res.json(LIVES);

    case "get_vod_info": {
      const movie = MOVIES.find((m) => String(m.stream_id) === String(req.query.vod_id)) || MOVIES[0];
      return res.json({
    ...
    case "get_series_info":
      return res.json({
        ...
        episodes: {
          "1": [
            { id: "301", episode_num: 1, title: "For Bigger Fun",
            ...
            { id: "302", episode_num: 2, title: "For Bigger Joyrides",
              container_extension: "mp4",
...
app.get("/movie/:user/:pass/:file", (req, res) => {
  const url = MOVIE_TARGET[Number(idFromFile(req.params.file))];
  return url ? res.redirect(302, url) : res.status(404).end();
});
app.get("/live/:user/:pass/:file", (req, res) => {
  const url = LIVE_TARGET[Number(idFromFile(req.params.file))];
  return url ? res.redirect(302, url) : res.status(404).end();
});
app.get("/series/:user/:pass/:file", (req, res) => {
  const url = EPISODE_TARGET[idFromFile(req.params.file)];
  return url ? res.redirect(302, url) : res.status(404).end();
});

app.get("/", (_req, res) =>
  res.type("text").send("Mock Xtream server — OK. Use any username/password."));

app.listen(PORT, () => console.log(`Mock Xtream server on :${PORT}`));
